...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	// add your code here
    ListNode *cur;
	int i; //index for frontlist
	int j; //index for backlist
	i=0;
	j=0;
	int s;
	s=1;
    cur = findNode(ll, 0); //cur points to head
    ...
    while (cur->next != NULL){
        s++; //to find out how many elements there are
        cur = cur->next; //cur points to next element
    }

    cur = findNode(ll, 0); //cur points to head again

	//to add elements into front list
    for (i=0; i<((s+1)/2); i++){
        insertNode(resultFrontList, i, cur->item);
        cur = cur->next;
    }

	//to add elements into back list
    for (i=(s+1)/2; i<s; i++){
		//start from j=0 for backlist
