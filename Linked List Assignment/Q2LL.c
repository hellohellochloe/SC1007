...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	// add your code here
	ListNode *cur;
	int s1; //size of list 1
	s1=ll1->size;
	int s2; //size of list2
	s2=ll2->size;
	int ms; //greatest no of elements > greatest index
	int num; //value of node *cur points to
    ...
    for (i=0; i<ms; i++){
        cur = findNode(ll2, 0);
		num = cur->item;
		//index increases in multiples of 2, so 2*i+1
		insertNode(ll1,(1+2*i), num);
        removeNode(ll2,0);
    }
